...
export default class MeshReflectorMaterial extends MeshStandardMaterial {
  ...
  setupBuffers(resolution, blur, bufferSamples) {
    const parameters = {
      ...
      // niente "encoding" qui: lo settiamo sulla texture
    };
  ...
  update() {
    ...
    const hasXR = !!this.gl.xr;
    const currentXrEnabled = hasXR ? this.gl.xr.enabled : false;
    const currentShadowAutoUpdate = this.gl.shadowMap.autoUpdate;

    this.beforeRender();
    if (hasXR) this.gl.xr.enabled = false;
    ...
    if (hasXR) this.gl.xr.enabled = currentXrEnabled;
